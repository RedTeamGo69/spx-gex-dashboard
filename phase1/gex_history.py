...
# List of every Postgres table this app writes to. Used by the "Reset all
# data" button. Order matters only for readability — TRUNCATE CASCADE isn't
# used and none of these tables have foreign key dependencies on each other.
ALL_APP_TABLES = [
    "gex_snapshots",
    "em_snapshots",
    "weekly_spx",
...
def reset_all_data():
    """
    TRUNCATE every Postgres table used by the app. Destructive — cannot be
    undone.

    Returns a dict mapping {table_name: rows_deleted} for tables that were
    successfully truncated, or {table_name: "error: <msg>"} for any table
    that failed (missing table, permission error, etc.). The call is
    best-effort — one failing table does NOT abort the others.
    """
    results: dict[str, object] = {}
    conn = _pg_get_connection()
    try:
        cur = conn.cursor()
        for tbl in ALL_APP_TABLES:
            try:
                # Count first so we can report how many rows were deleted
                cur.execute(f"SELECT COUNT(*) FROM {tbl}")
                row = cur.fetchone()
                count = int(row[0]) if row else 0
                cur.execute(f"TRUNCATE TABLE {tbl}")
                results[tbl] = count
                _logger.info(f"reset_all_data: truncated {tbl} ({count} rows)")
            except Exception as e:
                # Most common cause: the table doesn't exist yet (fresh DB).
                # Not worth aborting the rest of the reset — record and move on.
                results[tbl] = f"error: {e}"
                _logger.warning(f"reset_all_data: failed to truncate {tbl}: {e}")
    finally:
        conn.close()
    return results
